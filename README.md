...
> # Макагонов
# Changelog 
## 7.07.21 Была создана тестовая сцена, на которой проверена и реализована самостоятельная езда автомобиля по дорожному графу.
...
## 16.07.21 Определены границы трассы, размещён дорожный граф и перенесена модель машины
На выбранном участке был построен маршрут для ботов, также настроены некоторые параметры автомобиля, так как масштаб карты изменился в сравнение с тестовым.
Также были добавлены границы трассы, которые будут не допускать застревания в препятствиях или прохождения сквозь текстуры машиной.
![Граница трассы](https://github.com/Tamerlan91011/AUTO_RACE_UNITY/blob/main/Vid_gif_info/Screenshot_7.png)
> # Глазунов
# Changelog 
